...
_SYSTEM_PROMPT = (
    ...
    "但最多不要超過 6 步——多個候選可以合併成一步簡短帶過（例如「候選 A/B/C 均因 "
    "XX 條件不通過而排除」），不需要每個候選各自展開成一整步。\n"
    "3a. 精簡是硬性要求，不是建議：reasoningSteps 每一步的 detail 控制在 60 個字"
    "以內，title 控制在 15 個字以內；reasoning 整段不超過 200 字；public_message "
    "不超過 80 字。只寫判斷所需的關鍵數據與結論，不要重複解釋、不要加場面話。\n"
