...
def _escape_stray_quotes(text: str) -> str:
    ...
    field value inside a string instead of escaping it (e.g. `"...type
    精確等於 "Power_Failure"，或..."`), which is the actual cause behind
    most "Expecting ',' delimiter" fallbacks (not truncation, not a
    preamble)."""
    result = []
    in_string = False
    i = 0
