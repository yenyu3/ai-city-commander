...
// 市民端問題刻意用白話口吻、不帶 SOP 條號或門檻數字，貼近一般用路人會問的問題
...
export default function ChatPanel({
  ...
  const resolvedSuggestions =
    suggestions ?? (isPublic ? PUBLIC_SUGGESTIONS : GOVERNMENT_SUGGESTIONS);
  // 小人移動到新路段時視為新的 context，重新自動附加，讓 Tag 行為像「目前位置」而非一次性選取
  useEffect(() => {
    setContextDismissed(false);
  }, [fieldInspectorPosition?.nearestRoadId]);
  const attachedContext =
    !contextDismissed && fieldInspectorPosition?.nearestRoadId
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.title}>
        {title ??
          (isPublic
            ? pick(language, "市民問答", "Ask a Question")
            : pick(language, "對話式 SOP 問答", "SOP Q&A Chat"))}
      </div>

      <div className={styles.messages} ref={messagesRef}>
        {messages.length === 0 && resolvedSuggestions.length > 0 && (
          <div className={styles.suggestions}>
            {resolvedSuggestions.map((q) => (
              <button
                key={q}
          ...
          >
                  ...
                  return (
                    <div key={ref} className={styles.refWrap}>
                      <button
                        ...
                        onClick={() =>
                          setExpandedRef(expandedRef === key ? null : key)
                        }
                      >
            ...
            {!isPublic && m.reasoningSteps && m.reasoningSteps.length > 0 && (
              <div className={styles.reasoningWrap}>
                ...
                >
                  {pick(language, "查看推理過程", "View reasoning")} (
                      ...
                      .map((step) => (
                        <li key={step.order} className={styles.reasoningStep}>
                          <div className={styles.reasoningStepHead}>
                            <span className={styles.reasoningStepTitle}>
                              {step.title}
                            </span>
                            {step.sopRef && (
                              <span className={styles.reasoningStepRef}>
                                {step.sopRef}
                              </span>
                            )}
                          </div>
                          <p className={styles.reasoningStepDetail}>
                            {step.detail}
                          </p>
                        </li>
                      ))}
      ...
      <div className={styles.inputRow}>
        <input
          ...
          placeholder={
            placeholder ??
            (isPublic
              ? pick(language, "輸入您的問題…", "Ask a question…")
              : pick(language, "輸入假設性問題…", "Ask a what-if question…"))
          }
          value={input}
