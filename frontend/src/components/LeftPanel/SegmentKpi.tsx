...
export default function SegmentKpi() {
  ...
  const avgSat = list.length > 0 ? list.reduce((sum, s) => sum + s.saturation, 0) / list.length : 0;
  ...
  return (
    <div className={styles.grid}>
      <div className={styles.card}>
        <span className={styles.label}>{pick(language, "A 級路段", "Tier A")}</span>
        <span className={`${styles.value} ${styles.crit}`}>
          {critCount} <span className={styles.unit}>{pick(language, "條", "seg")}</span>
        </span>
      </div>
      <div className={styles.card}>
        <span className={styles.label}>{pick(language, "B 級路段", "Tier B")}</span>
        <span className={`${styles.value} ${styles.warn}`}>
          {warnCount} <span className={styles.unit}>{pick(language, "條", "seg")}</span>
        </span>
      </div>
      <div className={styles.card}>
        <span className={styles.label}>{pick(language, "平均飽和度", "Avg Sat.")}</span>
